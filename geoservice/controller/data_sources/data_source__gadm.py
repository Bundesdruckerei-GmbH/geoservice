...
class DataSourceGADM(DataSourceBase):
    ...
    def _local_storage_path(cls, qualities: NamedTuple) -> Path:
        return RESOURCES_PATH / 'gadm' / 'adm1.gpkg'

    @classmethod
    def _layer(cls, qualities: NamedTuple) -> str:
        return {
            "adm0": "GADM_ADM1_HIGHRES",
            "adm1": "GADM_ADM1_HIGHRES",
    ...
    def _extract(cls, qualities: Optional[NamedTuple] = None, fetch_mode: bool = False, **kwargs) -> GeoDataFrame:
        if not cls._check_adm_data('gadm', 'adm1', 0) or ((qualities.simplification_level == 0) and (qualities.adm_level == 'adm1')):
            gdf = super()._extract(qualities._replace(adm_level='adm1'), fetch_mode=fetch_mode, **kwargs)
            # - - - - - - - - - - - - - - - - - - - -
            return gdf
    ...
    def _transform(cls, gdf: GeoDataFrame, qualities: Optional[NamedTuple] = None) -> GeoDataFrame:
        if not cls._check_adm_data('gadm', 'adm1', 0) or ((qualities.simplification_level == 0) and (qualities.adm_level == 'adm1')):
            columns_to_keep_by_adm_level = {
                "adm0": ["GID_0", "NAME_0", "geometry"],
                "adm1": ["GID_0", "GID_1", "NAME_1", "geometry", "NAME_0"],
            }
            gdf.drop(set(gdf.columns) - set(columns_to_keep_by_adm_level['adm1']), axis=1, inplace=True)
            # - - - - - - - - - - - - - - - - - - - -
            gdf.rename(
                columns={"GID_0": "adm0_code", 
                        "GID_1": "adm1_code", 
                        "NAME_1": "name",
                        "NAME_0": "adm0_name"},
                inplace=True,
    ...
    def _persist(cls, gdf: GeoDataFrame, qualities: Optional[NamedTuple] = None) -> None:
        ...
        if not cls._check_adm_data('gadm', 'adm1', 0) or ((qualities.simplification_level == 0) and (qualities.adm_level == 'adm1')):
                if not cls._check_adm_data('gadm', 'adm1', 0):
